...
/* ── Web3Forms submission ─────────────────────────────────────
   AJAX submit to Web3Forms so the page never reloads and the
   custom UI is preserved. The access key lives in contact.html
   (hidden input) — buyers only swap that one value.
   ───────────────────────────────────────────────────────────── */
function initWeb3Forms() {
  ...
  const setStatus = (msg, type) => {
    ...
    status.className = 'ct-status ct-status--' + type;
    status.hidden = false;
  };
  ...
  form.addEventListener('submit', async (e) => {
    ...
    // Preserve native validation (required fields, email format) + accessibility
    ...
    try {
      ...
      if (data.success) {
        ...
        setStatus(data.message || 'Something went wrong. Please try again, or email us directly.', 'error');
      }
    } catch (err) {
